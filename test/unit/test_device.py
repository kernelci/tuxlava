...
def test_usbg_passes_the_deploy_os_to_both_deploys(tmp_path):
    ...
    job.initialize()
    deploys = [
        a["deploy"] for a in yaml.safe_load(job.render())["actions"] if "deploy" in a
    ]
    assert [d["to"] for d in deploys] == ["downloads", "usbg-ms"]
    assert [d["os"] for d in deploys] == ["oe", "oe"]
...
def test_usbg_runs_commands(tmp_path):
    job = usbg_job(tmp_path, commands=["uname", "-a"])
    job.initialize()
    assert "lava-test-case command --shell uname -a" in job.render()
    ...
    [
        ...
        (
            [
                ...
                "--secrets",
                "Authorization=Bearer all",
                "uefi:Authorization=Bearer uefi",
                "--bl1",
                "tf-bl1.bin",
                "--fip",
                "fip.bin",
        ...
        (
            [
                ...
                "--secrets",
                "Authorization=Bearer all",
                "fip:Authorization=Bearer fip",
                "--ap-romfw",
                "https://example.com/tf-bl1.bin",
                "--mcp-fw",
                "https://example.com/mcp_fw.bin",
                "--mcp-romfw",
                "https://example.com/mcp_romfw.bin",
                "--rootfs",
                "https://example.com/busybox.img.xz",
                "--scp-fw",
                "https://example.com/scp_fw.bin",
                "--scp-romfw",
                "https://example.com/scp_romfw.bin",
                "--fip",
                "https://example.com/fip.bin",
            ],
            "fvp-morello-busybox-secrets.yaml",
        ),
        (
            [
                "--device",
